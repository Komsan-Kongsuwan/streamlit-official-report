...
def render_chart_page():
    ...
    st.markdown("""
        <h3 style='margin-top:0; margin-bottom:0.5rem; color:#333;'>
            f"🕵️‍♂️ {st.session_state.selected_site} Comparison {prior_month.strftime('%B %Y')} vs {latest_month.strftime('%B %Y')}"
        </h3>
    """, unsafe_allow_html=True)
    ...
    for col, data in zip(cols, comparison_data):
        ...
        <div style="border:1px solid #ccc; border-radius:6px; padding:6px;
            ...
            <p style="margin:2px 0; font-size:12px;"><b>{data['Month1']}:</b> 
                <span style="color:blue;">{data['Current']}</span></p>
            <p style="margin-top:2px; color:{data['Color']}; font-weight:bold; font-size:12px;">

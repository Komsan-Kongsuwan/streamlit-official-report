...
# Step 1: Button triggers file picker
if "show_customer_uploader" not in st.session_state:
    st.session_state.show_customer_uploader = False

if st.button("📂 Generate Customer Report"):
    st.session_state.show_customer_uploader = True

# Step 2: File picker only shows after button click
if st.session_state.show_customer_uploader:
    ...
    if uploaded_files:
        ...
        def generate_customer_report(files):
            ...
            for i, file in enumerate(files):
                ...
                for j, row_index in enumerate(warehouse_rows):
                    site = extract_site(df.iloc[row_index, 0])
                    next_row = warehouse_rows[j+1] if j+1 < len(warehouse_rows) else len(df)
